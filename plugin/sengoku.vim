...
func! s:ProvDefs() abort
  return [
        ...
        \ ['出羽', '伊達', 1,10, ['陸奥','会津','越後'],            400, 30, 4000, {}],
        \ ['磐城', '相馬', 1,11, ['陸奥','会津','常陸'],            350, 30, 8000, {}],
        \ ['会津', '蘆名', 2,10, ['出羽','越後','常陸','上野','磐城'], 380, 30, 4000, {}],
        \ ['常陸', '佐竹', 2,11, ['会津','上野','武蔵','安房','磐城'], 420, 35, 4200, {}],
        \ ['上野', '北条', 3, 9, ['会津','常陸','武蔵','信濃','越後'], 400, 35, 4000, {}],
        \ ['武蔵', '北条', 3,10, ['常陸','上野','相模','甲斐','安房'], 520, 60, 5500, {}],
        \ ['相模', '北条', 4,10, ['武蔵','甲斐','駿河','安房'],     400, 65, 5000, {}],
        \ ['安房', '里見', 3,11, ['常陸','武蔵','相模'],            340, 40, 3800, {}],
        \ ['越後', '上杉', 2, 9, ['出羽','会津','上野','信濃','越中'], 450, 45, 6000, {}],
        \ ['越中', '上杉', 2, 8, ['越後','加賀','美濃'],            340, 35, 3200, {}],
        \ ['加賀', '本願寺', 2, 7, ['越中','越前'],                 400, 45, 4500, {'loyal': 90}],
        \ ['越前', '朝倉', 2, 6, ['加賀','近江','美濃'],            420, 50, 4200, {}],
        \ ['信濃', '武田', 3, 8, ['越後','上野','甲斐','美濃','三河','遠江'], 430, 30, 5000, {}],
        \ ['甲斐', '武田', 4, 9, ['信濃','武蔵','相模','駿河'],     360, 35, 5500, {}],
        \ ['駿河', '今川', 5, 9, ['甲斐','相模','遠江'],            400, 55, 4800, {}],
        \ ['遠江', '今川', 5, 8, ['駿河','信濃','三河'],            360, 40, 3500, {}],
        \ ['三河', '松平', 4, 8, ['遠江','信濃','尾張'],            380, 40, 4200, {}],
        \ ['尾張', '織田', 4, 7, ['三河','美濃','伊勢'],            540, 65, 5800, {}],
        \ ['美濃', '斎藤', 3, 7, ['信濃','越中','越前','尾張','近江'], 470, 45, 4800, {}],
        \ ['伊勢', '北畠', 4, 6, ['尾張','近江','大和','紀伊'],     440, 55, 4000, {}],
        \ ['近江', '浅井', 3, 6, ['越前','美濃','伊勢','山城'],     480, 60, 4500, {}],
        \ ['山城', '三好', 3, 5, ['近江','丹波','摂津','大和'],     400, 95, 4200, {}],
        \ ['摂津', '三好', 4, 4, ['山城','丹波','播磨','大和','紀伊','阿波'], 450, 90, 4500, {'port': 1}],
        \ ['大和', '松永', 4, 5, ['山城','摂津','伊勢','紀伊'],     400, 50, 4000, {}],
        \ ['紀伊', '雑賀', 5, 5, ['大和','摂津','伊勢','阿波'],     360, 50, 4200, {'guns': 80}],
        \ ['丹波', '波多野', 3, 4, ['山城','摂津','但馬','播磨'],   340, 35, 3200, {}],
        \ ['播磨', '赤松', 3, 3, ['丹波','但馬','摂津','備前'],     420, 50, 3800, {}],
        \ ['但馬', '山名', 2, 3, ['丹波','播磨','出雲'],            300, 35, 3000, {}],
        \ ['備前', '浦上', 4, 3, ['播磨','出雲','安芸'],            380, 45, 3600, {}],
        \ ['出雲', '尼子', 3, 2, ['但馬','備前','安芸','周防'],     400, 40, 4800, {}],
        \ ['安芸', '毛利', 4, 2, ['備前','出雲','周防','伊予'],     400, 50, 5200, {}],
        \ ['周防', '毛利', 4, 1, ['安芸','出雲','筑前','伊予'],     380, 55, 4200, {}],
        \ ['阿波', '三好', 5, 4, ['摂津','紀伊','土佐','伊予'],     380, 45, 4500, {}],
        \ ['土佐', '長宗我部', 6, 3, ['阿波','伊予'],               350, 35, 4500, {}],
        \ ['伊予', '河野', 5, 3, ['阿波','土佐','安芸','周防','豊後'], 360, 40, 3500, {}],
        \ ['筑前', '大友', 5, 1, ['周防','肥前','豊後'],            400, 75, 4200, {'port': 1}],
        \ ['豊後', '大友', 5, 2, ['筑前','伊予','日向'],            420, 55, 5000, {'guns': 30, 'port': 1}],
        \ ['肥前', '龍造寺', 5, 0, ['筑前','薩摩'],                 380, 45, 4200, {'port': 1}],
        \ ['日向', '伊東', 6, 1, ['豊後','薩摩'],                   340, 30, 3500, {}],
        \ ['薩摩', '島津', 6, 0, ['肥前','日向'],                   380, 35, 5200, {'guns': 30, 'port': 1}],
        \ ]
endfunc
...
func! s:LayoutStats() abort
  ...
  "
  " 41ヶ国を同じ大きさのマスで並べる都合上、隣接関係を配置だけで
  " 完全に表すことはできない。優先順位は次のとおり:
  "   1. 隣国が離れて描かれるのを避ける (見落とすと出陣できることに気付けない)
  "   2. 隣でないのに隣に描かれるのは許容する (出陣メニューを見れば分かる)
  " 行動中の国の隣国は ! / + の印で必ず示されるので、判断は印に従えばよい。
  let orth = 0
  let diag = 0
  let far = []
  for i in range(len(s:prov))
  ...
  " 辺を接して描かれるが隣国ではない組 (許容するが数は把握しておく)
  let cells = {}
  for i in range(len(s:prov))
    let cells[s:prov[i].gr . ',' . s:prov[i].gc] = i
  endfor
  let ghost = []
  for i in range(len(s:prov))
    let p = s:prov[i]
    for d in [[0, 1], [1, 0]]
      ...
      let j = cells[key]
      if index(p.adj, j) < 0
        call add(ghost, printf('%s-%s', p.name, s:prov[j].name))
      endif
    endfor
  endfor
  return {'orth': orth, 'diag': diag, 'far': far, 'ghost': ghost}
endfunc
...
func! sengoku#SelfTest() abort
  ...
  echom printf('MAP OK: %d国 %d家 隣接対称・グリッド衝突なし',
  ...
  echom printf('LAYOUT: 隣に描かれるが隣国でない組%d (許容) %s',
        \ len(st.ghost), empty(st.ghost) ? '' : '[' . join(st.ghost, ' ') . ']')
  " 武将データ
  let ngens = 0
  for ci in range(len(s:clans))
    if len(s:gens[ci]) != 8
